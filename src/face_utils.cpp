  ...
  )
{
  ...
  samples.reserve((face_box.width - patch_size + 1) * (face_box.height - patch_size + 1));
  int num_trees = forest.numberOfTrees();
  for (int x = face_box.x; x < face_box.x + face_box.width - patch_size; x += step_size)
  {
    for (int y = face_box.y; y < face_box.y + face_box.height - patch_size; y += step_size)
    {
  ...
  {
    e.submit(boost::bind(&Forest<HeadPoseSample>::evaluate_mt, forest, &samples[i], &leafs[i * num_trees]));
  }
  e.join_all();
};
  ...
  )
{
  ...
  samples.reserve((face_box.width - patch_size + 1) * (face_box.height - patch_size + 1));
  int num_trees = forest.numberOfTrees();
  for (int x = face_box.x; x < face_box.x + face_box.width - patch_size; x += option.step_size)
  {
    for (int y = face_box.y; y < face_box.y + face_box.height - patch_size; y += option.step_size)
    {
      cv::Rect patch_box(x, y, patch_size, patch_size);
  ...
  for (unsigned int i = 0; i < samples.size(); i++)
  {
    e.submit(boost::bind(&Forest<MPSample>::evaluate_mt, forest, &samples[i], &leafs[i * num_trees]));
  }
  e.join_all();
